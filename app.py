...
def handle_data_question_with_pandasai(state: GraphState) -> GraphState:
    """Handle data questions using PandasAI"""
    if not state.get('is_data_question'):
        return state

    print("Handling data question with PandasAI...")
    question = state['user_question']
    data_df = pd.DataFrame(state['data_sql'])

    try:
        # Initialize PandasAI for each question (could be optimized)
        smart_df = pai.SmartDataframe(data_df,config={"llm": llm})
        
        # Get response (could be text, dataframe, or visualization)
        ...
        if isinstance(response, pd.DataFrame):
            ...
            # For text responses, create a simple DataFrame
            results = pd.DataFrame({"Result": [str(response)]})
            
        return {
            **state,
            "data_analysis_results": results.to_dict(),
            "pandasai_response": str(response)
        }

    except Exception as e:
        print(f"PandasAI failed: {str(e)}")
        # Fallback to your original method if PandasAI fails
        return handle_data_question(state)
...
workflow.add_node("handle_data_question", handle_data_question_with_pandasai)
